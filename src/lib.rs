...
#[inline]
pub fn unsigned_num_to_hex(input_number: u64, buffer: &mut [u8]) {
...
#[inline]
pub fn unsigned_num_to_hex_tail(input_number: u64, buffer: &mut [u8; 10]) {
    let num_bytes = input_number.to_be_bytes();

    for (byte, dst) in num_bytes[3..].iter().zip(buffer.chunks_exact_mut(2)) {
        dst[0] = encode_nibble(byte >> 4);
        dst[1] = encode_nibble(byte & 0xF);
    }
}
...
mod tests {
    ...
    fn num_to_hex_max() {
        ...
        unsigned_num_to_hex(u64::MAX, &mut buffer);
    ...
    fn hex_to_num_random_full_buffer() {
        let mut buffer = vec![0u8; 16];
        let inputs: Vec<u64> = vec![696945551951787582, 14670462735960287247, 16883653885393442501, 18041941842666528556, 5835101380978880591,
    ...
    fn hex_to_num_random_non_full_buffer() {
        let mut buffer = vec![0u8; 16];
        let inputs: Vec<u64> = vec![2789444564, 695265371, 283150, 1916511949, 564194367,
